...
export function buildFingerprint(data: {
  ...
  // Round amount to 2 decimal places to avoid float drift
  const amt = Math.round(data.amount * 100);
...
export async function computePL(startDate: Date, endDate: Date): Promise<PLStatement> {
  ...
  const sumEbayFee = (feeType: Transaction["ebayFeeType"]) =>
    txns.filter(t => t.ebayFeeType === feeType).reduce((acc, t) => acc + Math.abs(t.amount), 0);
