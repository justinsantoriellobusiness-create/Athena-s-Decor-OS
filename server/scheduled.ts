...
export function registerScheduledRoutes(app: Router) {
  ...
  // ─── Autonomous Fulfillment: Shopify paid orders → CJ orders → tracking sync
  app.post("/api/scheduled/fulfillment", cronAuth, async (req, res) => {
    ...
    try {
      ...
      await updateAutomationSetting("fulfillment", {
        lastRunAt: new Date(),
        lastRunStatus: result.errors.length > 0 && result.ordersPlaced === 0 && result.ordersShipped === 0 ? "error" : "success",
        lastRunMessage: `placed=${result.ordersPlaced} shipped=${result.ordersShipped} skipped=${result.ordersSkipped}${result.errors.length ? ` errors=${result.errors.length}` : ""}`,
      });
      res.json({ success: true, ...result });
    } catch (err: any) {
      await updateAutomationSetting("fulfillment", { lastRunAt: new Date(), lastRunStatus: "error", lastRunMessage: err.message });
      res.status(500).json({ error: err.message });
    }
  });
